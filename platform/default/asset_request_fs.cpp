...
public:
    ...
    void open(const std::string& pathname, int flags, mode_t mode,
...
void AssetRequest::fileClosed(int) {
    MBGL_VERIFY_THREAD(tid);
...
void AssetRequest::close() {
    ...
    if (fd >= 0) {
        request = worker->invokeWithCallback(&FileIOWorker::close,
            std::bind(&AssetRequest::fileClosed, this, _1), fd);
        fd = -1;
    } else {
        cleanup();
    }
